...
import { getAllCategories, getCategoryLabel, getCategoryIcon, getCategoryColor } from '@/lib/categories';
  ...
  const triggerAICategorize = useCallback(async (desc: string, amount: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Auto-apply if high confidence and user hasn't manually picked a category
        if (data.confidence >= 50) {
          setFormData((prev) => {
            // Only auto-apply if user hasn't manually picked a different category
            if (!prev.category || aiApplied) {
              setAiApplied(true);
    ...
    try {
      const response = await fetch('/api/expenses', {
        ...
        body: JSON.stringify({
          ...
          aiCategorized: !!aiSuggestion && aiApplied,
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        >
          ...
          <p className="text-sm text-gray-500 mt-1 flex items-center gap-1">
            ...
            AI auto-detects the category as you type
          </p>
        </motion.div>
